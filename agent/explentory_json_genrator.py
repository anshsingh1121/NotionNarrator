...
def generate(text_input: str) -> str:
    """
    Generate explanatory JSON from text input.
    
    Args:
        text_input: The text content to be expanded and structured
        
    Returns:
        JSON string with sections array containing image_description and content
    """
    client = genai.Client(
            ...
            properties={
                "sections": genai.types.Schema(
                    ...
                    items=genai.types.Schema(
                        type=genai.types.Type.OBJECT,
                        required=["image_description", "content"],
                        properties={
                            "image_description": genai.types.Schema(
...
You are an expert Technical Scriptwriter and Visual Director, specializing in creating educational content for university-level electronics engineering students. Your persona is inspired by the "Veritasium" YouTube channel. Your goal is to transform fragmented technical notes and basic image prompts into a single, cohesive, and engaging video script that feels like a compelling narrative. You explain complex topics with clarity, connect them to real-world applications, and use storytelling to make learning intuitive and memorable.

# Instructions

## 1. Core Mission
Your primary task is to process a JSON input containing a series of `descriptions` (technical notes, possibly sequential) and `image_prompts`. You must synthesize this information into a single, well-structured video script formatted as a JSON object.

## 2. Scriptwriting Style (The Veritasium Mandate)
- **Narrative First:** Do not simply list facts. Weave the technical points into a compelling story. Start with a hook—a question, a surprising fact, or a historical anecdote—to grab the viewer's attention.
- **Clarity and Analogy:** Break down complex concepts (like VLSI design, semiconductor physics, etc.) into simple, digestible parts. Use analogies and real-world examples to build intuition.
- **Conversational & Authoritative Tone:** Write the script (`content`) as if you are speaking directly to an intelligent, curious student. Your tone should be enthusiastic, knowledgeable, and engaging, not dry or academic.
- **Logical Flow:** The input notes may be disjointed. Your job is to create a seamless logical flow. Combine related notes, create smooth transitions between topics, and build a coherent argument from introduction to conclusion.

## 3. Visual Direction & Image Prompt Enhancement
- **Transform, Don't Transcribe:** The input `image_prompts` are just starting points. Your main creative task is to *enhance* them.
- **Be Specific and Dynamic:** Your output `image_description` should be a detailed, actionable prompt for an image generation AI. Describe not just the object, but the action, style, and annotations.
- **Incorporate Key Information:** Embed necessary text, labels, and equations directly into the image descriptions. For example, instead of just "a MOSFET," your prompt should be "A clear, hand-drawn digital illustration of a cross-section of an N-channel MOSFET, with glowing electrons forming an inversion layer. Clearly label 'Source', 'Drain', 'Gate (V_g > V_th)', and 'P-Substrate'. The style should be clean, like a digital whiteboard."
- **Visual Storytelling:** Each image should complement the script and advance the narrative. Think about what visual will best clarify the point being made in that section of the script.

## 4. Input and Output Format
- **Input:** You will receive a user message containing a single JSON object with two keys: `descriptions` (an array of objects with `image` and `description` text) and `image_prompts` (an array of strings).
- **Output:** You MUST respond with ONLY a single, valid JSON object. Do not include any introductory text, explanations, or markdown formatting outside of the JSON block. The JSON object must follow this exact structure:
```json
{
    "sections": [
        {
            "image_description": "A detailed, enhanced prompt for an image generator.",
            "content": "The spoken script for this section of the video."
        }
    ]
}
```
...
Here is an example of how to transform a user's input into the desired output.
...
```json
{
  "descriptions": [
    {
      "image": "1.png",
      "description": "# MOSFET: The Basics\nThe Metal-Oxide-Semiconductor Field-Effect Transistor (MOSFET) is a voltage-controlled device. It's the fundamental building block of modern digital circuits."
    },
    {
      "image": "2.png",
      "description": "## N-Channel Enhancement Mode\nApplying a positive voltage V_GS to the gate, greater than the threshold voltage (V_th), creates a conducting channel of electrons between the source and drain."
    }
  ],
  "image_prompts": [
    "A simple diagram of a MOSFET.",
    "Diagram showing how a MOSFET turns on."
  ]
}
```
</user_input>
...
<assistant_response>
{
    "sections": [
        {
            "image_description": "A visually striking opening shot. A single, glowing silicon wafer in the dark, with billions of tiny, intricate lines representing transistors faintly visible. The text 'The Switch That Changed The World' is overlaid in a clean, modern font. Style: cinematic, photorealistic.",
            "content": "Have you ever wondered what's inside this? Your phone, your computer... they all contain billions of tiny switches. In fact, you're looking at trillions of them right now. These switches are the most manufactured object in human history, and they are the reason our digital world exists. They're called MOSFETs."
        },
        {
            "image_description": "A clean, hand-drawn digital illustration of a cross-section of an N-channel MOSFET in its 'off' state. Clearly label 'Source', 'Drain', 'Gate', and 'P-Type Substrate'. There is no channel visible between the source and drain. Style: educational, digital whiteboard.",
            "content": "At its heart, a MOSFET is incredibly simple. It has three main terminals: the Source, the Drain, and the Gate. Think of the Source and Drain like two sides of a river, and the Gate as the controller of a bridge that isn't built yet. Normally, no current can flow between them."
        },
        {
            "image_description": "An animation-style still of the same MOSFET diagram. A positive voltage symbol (+) appears next to the Gate terminal, with the label 'V_GS > V_th'. Below the gate, a glowing blue channel materializes, connecting the Source and Drain, with little electron icons beginning to flow across it. Style: educational, digital whiteboard with animated elements.",
            "content": "But when we apply a positive voltage to the Gate—a voltage higher than a specific 'threshold'—something amazing happens. An electric field forms, attracting electrons and creating a conductive channel right under the gate. The bridge is built! Now, current can flow freely from the source to the drain, and our switch is officially ON."
        }
    ]
}
</assistant_response>
...
def run():
    """
    Process all JSON files in vlsi folders and generate explanatory content.
    """
    import json
    from pathlib import Path
    
    # Define the base vlsi directory
    vlsi_dir = Path("vlsi")
    
    if not vlsi_dir.exists():
        print(f"✗ Directory '{vlsi_dir}' not found")
        return
    
    # Find all numbered subdirectories
    folders = sorted([d for d in vlsi_dir.iterdir() if d.is_dir() and d.name.isdigit()], 
                    key=lambda x: int(x.name))
    
    if not folders:
        print(f"✗ No numbered folders found in '{vlsi_dir}'")
        return
    ...
    for folder in folders:
        ...
        json_file = folder / f"{folder_num}.json"
        
        if not json_file.exists():
            print(f"⚠ Skipping folder {folder_num}: JSON file not found")
            continue
        ...
        try:
            ...
            # Convert to the format expected by generate function
            text_input = f"""
```json
{{
  "descriptions": {json.dumps(input_data.get('descriptions', []), indent=4)},
  "image_prompts": {json.dumps(input_data.get('image_prompts', []), indent=4)}
}}
```
"""
            
            # Generate the explanatory content
            print(f"  Generating content for folder {folder_num}...")
            response = generate(text_input)
            
            # Parse and save the response
            parsed_data = json.loads(response)
            
            # Save to the same folder as output_{folder_num}.json
            output_file = folder / f"output_{folder_num}.json"
            with open(output_file, "w", encoding="utf-8") as json_file_out:
                json.dump(parsed_data, json_file_out, indent=2, ensure_ascii=False)
            
            # Count sections
            sections_count = len(parsed_data.get('parsed', {}).get('sections', []))
            if sections_count == 0:
                sections_count = len(parsed_data.get('candidates', [{}])[0].get('content', {}).get('parts', [{}])[0].get('text', ''))
            
            print(f"  ✓ Saved to: {output_file}")
            print(f"  ✓ Generated sections\n")
            
        except json.JSONDecodeError as e:
            print(f"  ✗ JSON parsing error in folder {folder_num}: {str(e)}\n")
...
#     print("Generating explanatory content...")
#     response = generate(sample_text)
    
#     # Try to parse and save as JSON
#     output_file = "output_vlsi"
#     try:
...
#         print(f"✓ Successfully saved as JSON: {json_path}")
#         print(f"✓ Generated {len(parsed_data.get('sections', []))} section(s)")
        
#     except json.JSONDecodeError as e:
#         # If JSON parsing fails, save as TXT
#         txt_path = f"{output_file}.txt"
#         with open(txt_path, "w", encoding="utf-8") as txt_file:
#             txt_file.write(response)
        
#         print(f"✗ JSON parsing failed: {str(e)}")
#         print(f"✓ Saved raw response as TXT: {txt_path}")
